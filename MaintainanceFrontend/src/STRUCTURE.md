...
| `common/` | Cross-cutting app furniture: `PageHeader`, `EmptyState`, `ErrorState`, `ErrorBoundary/`, `SlaChip`, `Toaster`, `LocaleSwitch`, `StateBadge` (a record's state on the semantic surfaces — Live, Draft, Waiting) — and the **admin kit**: `CustomTable/`, `ResourceForm/`, `MediaPicker/`, `LocaleTabs`, `ConfirmDialog`, `RecordCombobox`, `FormDialog`, `RecordHistory`. See "The admin kit" below. |
