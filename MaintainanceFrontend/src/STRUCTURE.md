...
| `fields`, `schema`, `defaultValues` | ResourceForm fields, the zod schema from `form/schemas/cms.schema.js`, and a new record in API shape |
...
- **`ADMIN_NAV`** — seven groups in business order: **Overview · Sales · Operations · Finance · Aftercare · Content ·
  Platform**. An item is `{ to, label, icon, capability?, end?, soon? }`; `soon` renders greyed with a "Soon" tag,
  never as a link.
...
the unread count polls every minute; the list loads when the panel opens; opening an item marks it read and
navigates to `notificationHref(link)`; "Mark all as read" clears the badge.
