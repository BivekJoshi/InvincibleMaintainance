...
- **Phase G (2026-09-17):** backend unit 153 → **163** (message templates: placeholders, preview, address masking);
  ...
  rate card; invite without a password; `PUT` refuses a password; field-role profiles; an admin cannot disable,
  demote or delete themselves; the reset link never returned and redacted in the log; unlock; sessions listed without
  tokens and revoking them ends refresh; disabling ends sessions; login activity and its summary; template preview
  with missing vars, unsaved text and Devanagari; message-log filters, phone and email masking; retry audited and
  refused for a redacted link; SALES 403). All new tests ran red first. The API suite ran twice in a row, green both
  times. Two older tests changed because behaviour did: `11-logging-audit` checks password redaction through
  change-password, `05-ops` creates the technician profile's user as SALES. Frontend 313 → **355** in 43 files
  (SMS segments with English and Devanagari, the nested diff, record links, the capability matrix, message keys
  against the API's services, DataTable expandable / text filter / option groups, the History tab, every platform
  screen, the reset page). `npm run lint`: backend clean, frontend 0 errors (the same 25 warnings);
  `npm run build` succeeds; `npm run test:e2e` (the quotation loop) passes.
- **Phase G walk-through**, scripted with Playwright against its own API and Vite on the `_test` database (Chrome,
  1400 px), with screenshots checked: as admin@gharjatan.com.np — a quotation created, submitted, approved, sent and
  accepted by its link shows all five steps in the audit log filtered to it; opening the approval and "Show
  everything from this request" lists the approval, the job created and the status writes of that one request;
  sales@gharjatan.com.np locked with 5 bad sign-ins (the 6th, correct one refused) shows under Needs attention and
  as "Account locked", Unlock works and the correct password signs in again; the `lead_ack` Nepali SMS edited with
  `{{leadName}}` previews as "धन्यवाद Sita Rai, …", counts as Unicode at 70 a part, and saves; the admin's own row
  offers no Switch off; the roles page lists `quotations:approve`. As sales@gharjatan.com.np the nav has no Platform
  group, `/api/v1/admin/users` and `/api/v1/admin/audit-logs` answer 403, and `/admin/platform/users` sends them back
  to the dashboard.
